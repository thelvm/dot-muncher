...
func next_middle_tile() -> Vector2:
	...
	match current_direction:
		Vector2.UP:
			next_middle.x = Util.snap(position.x, tile_size, 0, offset)
			next_middle.y = Util.snap(position.y, tile_size, -1, offset)
		Vector2.DOWN:
			next_middle.x = Util.snap(position.x, tile_size, 0, offset)
			next_middle.y = Util.snap(position.y, tile_size, 1, offset)
		Vector2.LEFT:
			next_middle.x = Util.snap(position.x, tile_size, -1, offset)
			next_middle.y = Util.snap(position.y, tile_size, 0, offset)
		Vector2.RIGHT:
			next_middle.x = Util.snap(position.x, tile_size, 1, offset)
			next_middle.y = Util.snap(position.y, tile_size, 0, offset)
		_:
