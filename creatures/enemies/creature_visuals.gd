...
func _process(_delta: float) -> void:
	...
	if base_enemy._current_state == EnemyController.State.PANIC or base_enemy._current_state == EnemyController.State.GO_HOME:
	...
	if base_enemy._current_state == EnemyController.State.PANIC:
		self_modulate = Color.WHITE
		texture = _body_texture_vulnerable
	elif base_enemy._current_state == EnemyController.State.GO_HOME:
